...
ALTER TABLE hollow_raffles_users
  ADD COLUMN IF NOT EXISTS free_mint_reserved BOOLEAN NOT NULL DEFAULT FALSE;

CREATE INDEX IF NOT EXISTS idx_users_free_mint_reserved
  ON hollow_raffles_users(free_mint_reserved)
  WHERE free_mint_reserved = TRUE;

-- Reserve a free mint spot atomically with a hard cap.
CREATE OR REPLACE FUNCTION hollow_raffles_reserve_free_mint_spot(
  p_wallet VARCHAR(42),
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtext('hollow_raffles_free_mint_reserve'));

  INSERT INTO hollow_raffles_users (wallet_address)
